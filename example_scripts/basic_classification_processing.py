#Python 2.7.11 (default, May  5 2018)
#WAS Python 2.7.9 (default, Apr  5 2015, 22:21:35)
# the full environment I used to test this is in basic_project_stats.yml
import sys
import numpy as np  # works in 1.10.1 and 1.11.0
import pandas as pd  # works in 0.13.1 and 0.19.2
import datetime
...
def read_classfile(classfile_in, keep_allcols=False, verbose=True):
...
def isolate_workflow(classifications, workflow_id, workflow_version, workflow_ver_min=-1, workflow_ver_max=-1, verbose=True):
    ...
    return in_workflow, in_version


def remove_nonlive_classifications(classifications, verbose=True):
    ...
    return class_live, n_class_thiswf, n_live, n_notlive


def remove_duplicate_classifications(classfile_in, classifications, verbose=True):
    '''
    ...
    return classifications_nodups, n_class_nodup, n_dups, n_dup_pairs, duplicate_outfile
...
def basic_stats_processing(classfile_in, workflow_id=-1, workflow_version=-1, workflow_ver_min=-1, workflow_ver_max=-1, time_elapsed=False, output_csv=False, outfile_csv="", remove_duplicates=False, keep_nonlive=True, keep_allcols=False, verbose=True):
    ...
    classifications = read_classfile(classfile_in, keep_allcols, verbose)
    ...
    if (workflow_id > 0) | (workflow_version > 0) | (workflow_ver_min > 0) | (workflow_ver_max > 0):
        ...
        if verbose:
            print("Considering all classifications in workflow_versions:")
            print(version_ints)
    ...
    # get a quick classification count for ALL included versions
    workflow_ids = classifications.workflow_id.unique()
    version_ints = classifications.workflow_version.unique()
    ...
    if keep_nonlive:
        ...
        class_live, n_class_thiswf, n_live, n_notlive = remove_nonlive_classifications(classifications, verbose)
    ...
    if remove_duplicates:
        ...
        classifications_nodups, n_class_nodup, n_dups, n_dup_pairs, duplicate_outfile = remove_duplicate_classifications(classfile_in, classifications, verbose)
    ...
    if time_elapsed:
        ...
        # I'm not putting these print statements inside a verbosity check because there are no other outputs of the time computations, so if you are in this (if time_elapsed) statement and don't print anything you've wasted effort
    ...
    if output_csv:
        ...
        # this probably needs to be printed if it's written regardless of verbosity?
...
def basic_stats_help():
    ...
    print("    --silent")
    print("       by default, the program is verbose and many of the outputs are to screen. Flag this if you just want to print outfiles and not have any screen output.")
